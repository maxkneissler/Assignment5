...
```{r Spain, warning = FALSE, message = FALSE}
# Apply the query
...
# Filling into the final form (without longitude and latitude)
venueDataBE[(20 * 1 - 19):(1*page),] <- venueFilter
...
ggplot() +
  ...
  theme(title = element_text(size=8, face='bold'),
        plot.caption = element_text(face = "italic"))
  geom_point(venueDataBE, mapping = aes(longitude, latitude))
```

```{r Italy, warning = FALSE, message = FALSE}
# Apply the query
res_venuesGermany <- GET("https://app.ticketmaster.com/discovery/v2/venues",
                       query = list(countryCode = "DE",
                                   apikey = tm_api))

...
is conducted that the structure equals the output dataset on the assignment 
sheet.
...
As we see, there results besides the first two entries only missing values for
the longitude and the latitude of the venues. To check, what the issue is, we 
have a look at the particular `ID` of the venue to figure out if it is an issue
with our request or with the data.
...
As mentioned in the description of the GET-request, we could access 4738 
...
pages <- floor(entries/20)  # number of complete pages
remainder <- entries - 20*pages
```
...
created, we start to loop through the different pages. The remaining 
observations are added manually to prevent issues with filling the table. Like
observed in the exercise 3, it is not possible to extract the longitude and
...
venueDataComplete[(20 * page - 19):(20*page),] <- venueFilter
...
res_venuesAll <- GET("https://app.ticketmaster.com/discovery/v2/venues",
                       query = list(countryCode = "DE",
                                    page = page,
                                   apikey = tm_api))
res_venuesAll <- GET(query)
contentAll <- fromJSON(content(res_venuesGermany, as = "text"))
...
# Filling into the final form (without longitude and latitude)
venueDataComplete[(20 * page - 19):(entries),] <- venueFilter
```
...
# Filling into the final form (without longitude and latitude)
venueDataBE[(20 * page - 19):(20*page),] <- venueFilter
...
# Filling into the final form (without longitude and latitude)
venueDataBE[(20 * page - 19):(entries),] <- venueFilter
```
...
ggplot(venueDataBE, mapping = aes(longitude, latitude)) +
  geom_polygon(
    aes(x = long, y = lat, group = group), 
...
We will apply the same procedure to Austria.


```{r Austria, warning = FALSE, message = FALSE}
# Apply the query
res_venuesAT <- GET("https://app.ticketmaster.com/discovery/v2/venues",
                       query = list(countryCode = "AT",
                                   apikey = tm_api))

# Extract the content
contentAT <- fromJSON(content(res_venuesAT, as = "text"))
...
# Filling into the final form (without longitude and latitude)
venueDataAT[(20 * page - 19):(20*page),] <- venueFilter
...
# Filling into the final form (without longitude and latitude)
venueDataAT[(20 * page - 19):(entries),] <- venueFilter
```

As the data is extracted, we make once again use of the map to plot the borders
with the venues.

```{r Austria_plot, warning = FALSE, message = FALSE}

ggplot() +
  geom_polygon(
    aes(x = long, y = lat, group = group), 
                  data = map_data("world", region = "Austria"),
    fill = "grey90",color = "black") +
  theme_void() + coord_quickmap() +
  labs(title = "Event locations across Germany", 
                            caption = "Source: ticketmaster.com") +
  theme(title = element_text(size=8, face='bold'),
        plot.caption = element_text(face = "italic"))
  geom_point(venueDataAT, mapping = aes(longitude, latitude))
```

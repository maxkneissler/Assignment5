...
since they may vary over time as a flexible function
...
Then, we create a data frame with the entries, which we like to collect, to 
make the request faster. As an empty table is created, we start to loop through
the different pages. The remaining observations are added manually to prevent
issues with filling the created table.
...
query <- 
  paste0(
    "https://app.ticketmaster.com/discovery/v2/venues.json?countryCode=DE&page=+",
           page,"apikey=",tm_api)
  
res_venuesAll <- GET(query)
contentAll <- fromJSON(content(res_venuesGermany, as = "text"))
  
# choose the wanted sizes
venueFilter <- tibble(contentAll[["_embedded"]][["venues"]])
  
# extract only the desired entries
venueFilter <- venueFilter[,c(1, 10, 8, 12, 5)]
# change the coding
venueFilter$city <- as.character(venueFilter$city)
venueFilter$address <- as.character(venueFilter$address)
# add longitude and latitude
venueFilter$latitude <- 
            contentAll[["_embedded"]][["venues"]][["location"]][["latitude"]] 
venueFilter$longitude <- 
            contentAll[["_embedded"]][["venues"]][["location"]][["longitude"]]
  
venueDataComplete[(20 * i - 19):(20 * i),] <- venueFilter
...
venueDataComplete[(20 * i - 19):(entries),] <- venueFilter
```
...
events in Germany. In the assignment, it is already given a map-code of Germany,

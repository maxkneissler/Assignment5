...
indicating the German event locations. We will notice, however, that the 
corresponding longitude and latitude data for Germany is not available. 
Therefore, the procedure is repeated for Belgium and Denmark.
...
We see that the longitude and latitude variable are missing values for all 
venues besides the first two entries. In order to elaborate, what the issue is, 
we have a look at the particular `ID` of the venue to figure out if the issue is 
due to our request or due to the data.

```{r extract_id, warning = FALSE, message = FALSE}
# Check content via ID
...
By switching between the different `ID`s, one can notice that the variable 
`location` disappears for the IDs after the first two venue observations. 
Therefore, we can conclude that there is an issue with the data and not our 
query. As we will now want to extract data from several pages, we might end up 
with issues concerning the stacking of the information of the different pages as 
the `location` variable vanishes.
...
a size of 100 that we do not exceed for sure the request limit per day. The 
remaining observations on the last incomplete page are added manually to prevent 
issues with filling the table. Like observed in exercise 3, it is not possible 
to extract the longitude and latitude characteristic for the German venues. 
...
# Plot of Germany without the venue coordinates
ggplot() +
  geom_polygon(
...
query to Belgium and Denmark to plot the venues in the respective map. Just a 
few adjustments are necessary to obtain the desired result.
...
Belgium. In order to plot only the venues which are actually located in 

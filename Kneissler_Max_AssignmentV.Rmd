...
Europe. We will make use of this data by plotting a map of Germany, indicating 
the event locations in Germany. We will notice, however, that the corresponding
longitude and latitude data for Germany is not available. Therefore, the 
procedure is repeated for Belgium and Denmark.
...
`ticketmaster.com`, we see that the root URL requires a package, a version, an
...
security. Thus, we get used to this structure for personal keys on other APIs.
...
success of our query, we have a look at the status of the request before 
...
and `page`. We can observe that more than 4740 elements on 238 pages were 
imported. The folder `venues` on the other hand contains several subfolders like 
images, city, country, name, type, id, etc. In order to follow the assignment, 
we are interested in extracting the seven characteristics `name`, `city`, 
`postalCode`, `address`, `url`, `longitude` and `latitude`. In addition, some 
proper recoding is conducted that the structure equals the output on the 
assignment sheet.
...
As we see, the longitude and latitude variable are missing values for all venues 
besides the first two entries. To check, what the issue is, we have a look at 
the particular `ID` of the venue to figure out if the issue is due to our 
request or due to the data.
...
different pages as the `location` variable vanishes.
...
observations on the last incomplete page are added manually to prevent issues 
with filling the table. Like observed in exercise 3, it is not possible to 
extract the longitude and latitude characteristic for the German venues. 
Therefore, I exclude those properties for the query. We will then not be able to 
draw a map with the respective German venues.
...
# Fill in final form (without longitude and latitude)
...
characteristics available, the German map remains empty without the locations
of German venues
...
# Plot of Germany without the venue coordiantes
ggplot() +
  geom_polygon(
    aes(x = long, y = lat, group = group), 
                  data = map_data("world", region = "Germany"),
    fill = "grey90",color = "black") +
#   geom_point(plotGER, mapping = aes(longitude, latitude)) +
...
ggplot() +
  geom_polygon(aes(x = long, y = lat, group = group), 
                  data = map_data("world", region = "Belgium"),
                    fill = "grey90",color = "black") +
  geom_point(data = plotBE, aes(longitude, latitude), alpha = 0.2) +
  theme_void() + 
  coord_quickmap() +
...
Antwerp, Ghent and Liège. A few events are also located near the ocean.
